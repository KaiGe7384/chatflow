...
const PrivateMessageBubble: React.FC<PrivateMessageBubbleProps> = ({
  message,
  isOwnMessage,
  showAvatar,
  onDeleteMessage
  ...
  const handleDeleteMessage = () => {
    if (window.confirm('确定要删除这条私聊消息吗？删除后无法恢复。')) {
      onDeleteMessage && onDeleteMessage(message.id);
    }
  };

  const avatar = isOwnMessage ? message.sender_avatar : message.receiver_avatar;
  const username = isOwnMessage ? message.sender_username : message.receiver_username;
